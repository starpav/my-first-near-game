...
onready var timer_label = get_parent().get_node("TimerLebel")
...
func game_over(coin_value):
	if wallet_connection.is_signed_in():
		var args = {"receiver_id": wallet_connection.get_account_id(),\
					"amount": str(coin_value * 100000000)}
		wallet_connection.call_change_method(CONTRACT_NAME, "ft_mint", args)
...
func _on_QuitButton_pressed():
	var coins_value = $ColorRect/CoinsCounter.coins_collected
	game_over(coins_value)
